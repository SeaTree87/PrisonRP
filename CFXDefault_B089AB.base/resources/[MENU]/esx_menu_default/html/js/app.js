				...
				'<div class="menu-items">' + 
					'{{#elements}}' +
						'<div class="menu-item {{#selected}}selected{{/selected}}">' +
							'{{#hasIcon}}<img src="{{{icon}}}">{{/hasIcon}}<div>{{{label}}}{{#isSlider}}: <span>&lt; {{{sliderLabel}}} &gt;</span><input type="text" style="display: none;">{{/isSlider}}</div>{{#isShortcut}}<div class="shortcut">{{{shortcut}}}</div>{{/isShortcut}}' +
						'</div>' +
					'{{/elements}}' +
				'</div>'+
			'</div>' +
